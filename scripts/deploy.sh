...
terraform init -reconfigure -backend-config="$ENVIRONMENTS_FOLDER/$ENVIRONMENT/backend_$COMPONENT.tfvars"
terraform apply -auto-approve -lock=false -var-file="$ENVIRONMENTS_FOLDER/$ENVIRONMENT/variables.tfvars"
